...
contents through the reply. ``--ignore-user-config``/``--ignore-rules`` stop it loading hooks, MCP
servers, or project instructions from outside a fresh, empty, per-call scratch directory. See the
README's Security note before running this backend on a host with anything sensitive reachable.
"""
...
class CodexChatAnswer(BaseChatAnswer):
    ...
    def _complete(cls, prompt: str) -> str:
        ...
        with tempfile.TemporaryDirectory() as scratch_dir:
            ...
            run_cli(argv, prompt, timeout=settings.llm_timeout, cwd=scratch_dir)
